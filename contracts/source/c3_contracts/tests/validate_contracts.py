#!/usr/bin/env python3
"""Dependency-free C3 validator for the controlled JSON Schema subset."""
...
ROOT = HERE.parents[1] if (HERE.parents[1] / "contract-manifest.json").exists() else HERE.parents[1] / "outputs/ux_ui_phase6/c3_contracts"
C1_ROOT = ROOT.parent / "c1_contracts"
C2_ROOT = ROOT.parent / "c2_contracts"
FORBIDDEN_PUBLIC_KEYS = {
    "hidden_truth", "exact_dc", "private_motive", "private_actor_motive",
    "secret_drive", "raw_validator", "chain_of_thought",
    "exact_canonical_coordinates", "private_validator_diagnostic",
    "exact_bond", "obedience_score", "secret_front_clock",
    "hidden_feature_count", "hidden_immunity", "undelivered_fact",
    "future_consequence", "private_prompt", "prompt_text",
}
...
c2_schema_paths = sorted((C2_ROOT / "schemas").rglob("*.schema.json"))
c3_schema_paths = sorted((ROOT / "schemas").rglob("*.schema.json"))
for schema_path in c1_schema_paths + c2_schema_paths + c3_schema_paths:
...
for item in manifest["fixtures"]:
    ...
    leaked = sorted(set(walk_keys(document)) & FORBIDDEN_PUBLIC_KEYS) if item["expected_valid"] and item.get("public_payload", True) else []
    passed = actual_valid == item["expected_valid"] and not leaked
    results.append({
        "file": item["file"], "expected_valid": item["expected_valid"],
        "actual_valid": actual_valid, "knowledge_leak_keys": leaked,
        "passed": passed, "errors": errors[:10],
    })
...
openapi_text = verify_api_text("openapi/mythic-ui-c3.yaml", "openapi: 3.1.2")
asyncapi_text = verify_api_text("asyncapi/mythic-events-c3.yaml", "asyncapi: 3.1.0")
...
assert len(operation_ids) == 26, f"Expected 26 operations, found {len(operation_ids)}"
assert openapi_text.count("{ $ref: '#/components/parameters/IdempotencyKey' }") == 12
assert "receiveAsyncMediaEvents:" in asyncapi_text
async_message_count = len(re.findall(r"^    [A-Za-z][A-Za-z0-9]+:\n      name:", asyncapi_text, flags=re.MULTILINE))
assert async_message_count == 13, f"Expected 13 AsyncAPI messages, found {async_message_count}"

contract_manifest = load_json(ROOT / "contract-manifest.json")
assert all(item["status"] == "approved" for item in contract_manifest["approved_dependencies"])
assert contract_manifest["implementation_authorized"] is False
assert contract_manifest["status"] == "candidate"
...
report = {
    "checkpoint": "C3",
    "validator": "dependency-free deterministic JSON Schema 2020-12 subset used by C1-C3",
    "c1_dependency_schema_count": len(c1_schema_paths),
    "c2_dependency_schema_count": len(c2_schema_paths),
    "c3_schema_count": len(c3_schema_paths),
