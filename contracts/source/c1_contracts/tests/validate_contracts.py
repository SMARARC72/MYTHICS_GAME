#!/usr/bin/env python3
"""Dependency-free C1 contract validator for the JSON Schema subset used here."""
...
ROOT = HERE.parents[1] if (HERE.parents[1] / "contract-manifest.json").exists() else HERE.parents[1] / "outputs/ux_ui_phase6/c1_contracts"
FORBIDDEN_KEYS = {
    "hidden_truth",
    "exact_dc",
    "private_motive",
...
def validate(instance, schema, store, current_root, path="$", seen=None):
    ...
    if "oneOf" in schema:
        ...
        for child in schema["oneOf"]:
            child_errors = validate(instance, child, store, current_root, path, seen)
            if not child_errors:
...
openapi_text = verify_api_text("openapi/mythic-ui-c1.yaml", "openapi: 3.1.2")
asyncapi_text = verify_api_text("asyncapi/mythic-events-c1.yaml", "asyncapi: 3.1.0")
...
assert "name: Idempotency-Key" in openapi_text
assert "receiveViewpointEvents:" in asyncapi_text
...
report = {
    "checkpoint": "C1",
    "validator": "dependency-free deterministic JSON Schema 2020-12 subset used by C1",
